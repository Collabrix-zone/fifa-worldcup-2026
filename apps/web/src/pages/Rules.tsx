...
// Scoring cards stay in code because the point values are the same numbers
// the server uses to score predictions. Hand-edited values would drift
// from the leaderboard. The admin-editable section at the top is the
// place to add announcements / tournament-specific notes.
...
const BASE_RULES = [
  ...
  },
  {
    ...
    example:
      "You predict 2–1 (Team A wins). Actual is 0–1 (Team B wins, but you got Team B's score right).",
    tone: "muted",
    Icon: TrendingUp,
  },
  {
...
const KO_RULES = [
  ...
  },
  {
    ...
    example:
      "You predict 5–4 pens (Team A wins shootout). Actual: 4–2 pens, Team A wins. +3.",
  },
] as const;
...
export default function Rules() {
  // Admin-editable announcement / intro / notes block. Empty → hidden.
  // The scoring sections below are hard-coded so the displayed numbers
  // always match the server's scoring logic.
  const tournament = useGetTournament(TOURNAMENT_SLUG);
  const customNotes = (tournament.data?.rulesMd ?? "").trim();
      ...
      {customNotes && (
        ...
        >
          <div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary">
            <Sparkles className="w-3.5 h-3.5" /> From the admin
          </div>
          <article className="prose prose-invert prose-sm md:prose-base max-w-none prose-headings:text-white prose-strong:text-white prose-a:text-primary">
            <ReactMarkdown>{customNotes}</ReactMarkdown>
          </article>
        </section>
      )}
